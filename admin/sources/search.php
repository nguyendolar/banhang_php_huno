<h2 class="text-center p-3 text-success">Kết Quả Tìm Kiếm</h2>
<table class="table table-striped" width="100%" border="1px">
			...
			$stt=0;
			  $dt->select("SELECT * FROM product WHERE name_product LIKE '%$search_text%'  ORDER BY id desc limit $pag,10 ");
			while ($row=$dt->fetch()) {
				$stt++;
				?>
				<tr>
	...
	<div style="background: none" class="card-footer clearfix container">
		<div class="row">
			<ul class="pagination pagination-sm m-0 float-right">
				...
				<?php
					$dt->select("SELECT* FROM product WHERE name_product LIKE '%$search_text%' ");
					$count=$dt->num_rows();
